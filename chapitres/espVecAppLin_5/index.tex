...
\paragraph{Exemple} Soient $\vec{v_1}, \vec{v_2} \in \R^2$ données par $\vec{v_1}=(1,2), \vec{v_2}=(-3,7)$. Alors $\vec{w}=(-11,17) \in \R^2$ satisfait $\vec{w}=-2\vec{v_1}+3\vec{v_2}$ donc $\vec{w}$ est combinaison linéaire de $\vec{v_1}$ et $\vec{v_2}$. %TODO
%
...
\paragraph{Example} Soit $f: \R^3 \rightarrow \R^2$ définie par
$$f(x, y, z) = (2x + y - %z ,  x + 3y)$$
f est linéaire. \\
